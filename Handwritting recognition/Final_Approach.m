...
for j = j
 ...
 for i =0:2
    ...
    m = mean(Xtrain_k);
    [coeff score] = princomp(Xtrain_k);
centeredTrain = bsxfun(@minus,Xtrain_k,m);
centeredTrainInPCAcoords = centeredTrain*coeff;

centeredSample = bsxfun(@minus,Xtest_k,m);
centeredSampleInPCAcoords = centeredSample*coeff;
   
classTrain = knnclassify(centeredTrainInPCAcoords,score,Ytrain_k,3);
classTest = knnclassify(centeredSampleInPCAcoords,score,Ytrain_k,3);
...
title('KNN PCA Bias-Variance Trade off')
